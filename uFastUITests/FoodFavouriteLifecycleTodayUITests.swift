...
extension FoodFavouriteLifecycleUITests {
    ...
    func testTodayFavouriteCreatesSuccessStateAndActiveFastCancelClearsStateBeforeRetryingSelection() {
        ...
        XCTAssertTrue(foodAdd.waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
        foodAdd.tap()
        let favourite = app.buttons["food.favourite.10300000-0000-0000-0000-000000000001"]
        XCTAssertTrue(favourite.waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
        favourite.tap()
        XCTAssertTrue(app.staticTexts["Overnight oats"].waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
        XCTAssertTrue(
            identifiedInApp("food.favourite.success", in: app).waitForExistenceIfNeeded(timeout: 5),
        ...
        XCTAssertTrue(activeFavourite.waitForExistenceIfNeeded(timeout: 5), active.debugDescription)
        activeFavourite.tap()
        let confirmation = active.descendants(matching: .any)
            .matching(identifier: "food.favourite.confirmation").firstMatch
        XCTAssertTrue(confirmation.waitForExistenceIfNeeded(timeout: 5), active.debugDescription)
        let savingState = active.descendants(matching: .any)
            .matching(identifier: "food.favourite.commit-state").firstMatch
        XCTAssertTrue(savingState.waitForExistenceIfNeeded(timeout: 5), active.debugDescription)
        ...
        XCTAssertTrue(cancelConfirmation.waitForExistenceIfNeeded(timeout: 5), active.debugDescription)
        XCTAssertTrue(primaryConfirmation.waitForExistenceIfNeeded(timeout: 5), active.debugDescription)
        XCTAssertTrue(consequence.waitForExistenceIfNeeded(timeout: 5), active.debugDescription)
        ...
        XCTAssertTrue(active.staticTexts["fast.elapsed"].waitForExistenceIfNeeded(timeout: 5), active.debugDescription)
        ...
        XCTAssertTrue(confirmation.waitForNonExistence(timeout: 5), active.debugDescription)
        XCTAssertTrue(
            identifiedInContainer("food.favourite.success", in: active).waitForExistenceIfNeeded(timeout: 5),
            active.debugDescription
        )
        XCTAssertTrue(
            active.staticTexts["Overnight oats"].waitForExistenceIfNeeded(timeout: 5),
            active.debugDescription
        )
    }
    ...
    func testTodayActiveFastFoodFavouriteFailureKeepsFastAndProvidesRetry() {
        ...
        let confirmation = app.descendants(matching: .any)
            .matching(identifier: "food.favourite.confirmation").firstMatch
        XCTAssertTrue(confirmation.waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
        ...
        XCTAssertTrue(
            identifiedInApp("food.favourite.save-error", in: app).waitForExistenceIfNeeded(timeout: 5),
            app.debugDescription
        )
        XCTAssertTrue(app.buttons["food.favourite.retry"].waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
    }
    ...
    func testTodayActiveFastFavouriteStaleAfterConfirmationShowsScopedStateAndRetainsRetry() {
        ...
        XCTAssertTrue(primary.waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
        primary.tap()
        XCTAssertTrue(confirmation.waitForNonExistence(timeout: 5), app.debugDescription)

        XCTAssertTrue(
            app.staticTexts["food.favourite.stale"].waitForExistenceIfNeeded(timeout: 5),
            app.debugDescription
        )
        XCTAssertTrue(
            app.staticTexts["food.favourite.save-error"].waitForExistenceIfNeeded(timeout: 5),
            app.debugDescription
        )
        XCTAssertTrue(app.buttons["food.favourite.retry"].waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
        XCTAssertTrue(app.staticTexts["fast.elapsed"].waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
        ...
        XCTAssertTrue(
            app.staticTexts["food.favourite.success"].waitForExistenceIfNeeded(timeout: 5),
            app.debugDescription
        )
        XCTAssertTrue(app.staticTexts["Overnight oats"].waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
        XCTAssertTrue(app.staticTexts["fast.inactive-state"].waitForExistenceIfNeeded(timeout: 5), app.debugDescription)
    }
    ...
    func testFoodFavouriteControlsRemainReadableAtLargeTypeInDarkAppearance() {
        ...
        tapSettings(in: app)
        XCTAssertTrue(
            app.staticTexts["settings.food-favourites"].waitForExistenceIfNeeded(timeout: 5),
            app.debugDescription
        )
