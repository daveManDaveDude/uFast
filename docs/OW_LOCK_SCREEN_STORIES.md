...
**Current-state note (11 August 2026):** This was the historical OW-L102 scope.
D-031 now requires the implemented small, medium and large Home Screen widget
families alongside accessory rectangular. Dynamic Island remains an optional
Live Activity surface with compact, minimal and expanded regions, but there is
no persistent banner requirement or promise.
...
- **uFast**, **Elapsed**, the system-driven elapsed timer, the date-relative
  clamped progress track and stable goal context such as **16-hour goal**;
...
- **Lock Screen:** prioritize **uFast**, **Elapsed**, a system-driven elapsed
  timer, the date-relative clamped progress track, stable goal context such as
  **16-hour goal** and target time. Do not render a request-time sampled
  numeric percentage: ordinary text cannot remain current while the app and
  extension are suspended. Use the calm sky/evergreen visual relationship from
  the active-fast hero within system margins and legibility constraints.
...
- **Expanded:** elapsed is primary; the date-relative progress track, stable goal
  context and target are secondary. Every tappable region uses
  `ufast://fast/current` and performs no inline edit, end or goal action.
...
- **Accessibility:** expose one coherent value such as **uFast, elapsed 12 hours
  34 minutes, 16-hour goal, target 8:00 PM. Opens uFast.** Do not expose a
  request-time sampled percentage or cause recurring VoiceOver announcements as
  seconds advance. Meaning must not depend only on color, Dynamic Island
  geometry or punctuation. If a future public system-driven numeric treatment
  is proven current on a suspended physical iPhone, it may be reconsidered by a
